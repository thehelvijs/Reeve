...
// AccessRequest is a user's request for a host's credentials.
...
func (db *DB) CreateAccessRequest(hostID, requesterID, note string) (AccessRequest, error) {
	var existing int
	err := db.sql.QueryRow(
		`SELECT 1 FROM access_requests WHERE host_id = ? AND requester_id = ? AND status = 'pending'`,
		hostID, requesterID).Scan(&existing)
	if err == nil {
		return AccessRequest{}, ErrDuplicateRequest
	}
	r := AccessRequest{
		ID: NewID(), HostID: hostID, RequesterID: requesterID,
		Status: RequestPending, Note: note, CreatedAt: time.Now().UTC(),
	}
	_, err = db.sql.Exec(
		`INSERT INTO access_requests(id, host_id, requester_id, status, note, created_at)
		 VALUES (?,?,?,?,?,?)`,
		r.ID, r.HostID, r.RequesterID, r.Status, r.Note, r.CreatedAt.Format(time.RFC3339Nano))
	if err != nil {
...
const requestSelect = `SELECT id, host_id, requester_id, status, note, decided_by, decided_at, created_at FROM access_requests`
...
func scanRequest(row scanner) (AccessRequest, error) {
	...
	if err := row.Scan(&r.ID, &r.HostID, &r.RequesterID, &r.Status, &r.Note, &decidedBy, &decidedAt, &created); err != nil {
		return AccessRequest{}, err
	}
